;(function($){
  var Score = function(obj,option){
    ...
    this.defaultIcon.hover(function(e){
      ...
      $(this).parents("[class^='score-expression']").children('.score-description').html(self.selectLevelDescript(level));
      var _index = $(this).parents("[class^='score-expression']").index() - 1;

      for(var i = 0 ; i<self.len ; i++){
        if(i < level){
          $(this).parents("[class^='score-expression']").find('.default-icon').eq(i).css({
            "backgroundPositionX":-self.eachWidth * (level-1),
            "backgroundPositionY":-self.eachHeight * _index
          });
        }else{
          $(this).parents("[class^='score-expression']").find('.default-icon').eq(i).css({"backgroundPositionX":"right"});
        }
      }
    },function(e){
        ...
        for(var i = 0 ; i<self.len ; i++){
          if(i < parseInt(customLevel)){
            $(this).parents("[class^='score-expression']").find('.default-icon').eq(i).css({"backgroundPositionX":-self.eachWidth * (level-1)});
          }else{
            $(this).parents("[class^='score-expression']").find('.default-icon').eq(i).css({"backgroundPositionX":"right"});
          }
        }
